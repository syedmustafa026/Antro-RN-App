...
const Birthday = ({ navigation }: any) => {
  const goBack = () => {
    navigation.goBack();
  };

  const btnContinueHandler = () => {
    navigation.navigate(SCREEN_NAMES.designation);
  };

  return (
    <LoginBase title={"Your Birthday"} onBackIconPress={goBack}>
      <Text color={"#8391A1"} mt={"3"} fontSize={"16"} fontWeight={"medium"}>
        {"Your new password must be unique from those previously used."}
      </Text>
      <Box mt={"3.5"}>
        <TextInput placeholder={"Select your DOB"} />
      </Box>
